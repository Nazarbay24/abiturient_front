...
<!--                            <div class="card-category">Тенденция поступления новых заявок</div>-->
            ...
            <v-row>
                <v-col cols="12">
                    <div class="card full-height">
                        <div class="card-body">
                            <div class="card-title">Статистика по квотам</div>
                            <div class="row py-3">
                                <v-col>
                                    <apexchart v-if="series4.length" width="100%" height="600" type="bar" :options="options4" :series="series4"></apexchart>
                                </v-col>
                            </div>
                        </div>
                    </div>
...
<script>
    export default {
        ...
        methods: {
            ...
            getQuotaData() {
                this.$http.get('/request/quotaResult').then((response) => {
                    this.series4 = [response.data.data.body.values];
                    this.options4 = {...this.options4, ...{
                            xaxis: {
                                categories: response.data.data.body.category
                            }
                        }};
                }).catch(() => {});
