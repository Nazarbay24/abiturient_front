		...
		<div class="panel-header panel-header-custom bg-primary-gradient">
			<div class="page-inner py-5">
				<div class="d-flex align-items-left align-items-md-center flex-column flex-md-row">
					<div>
						<h2 class="text-white pb-2 fw-bold">{{ $t('Создать новую заявку') }}</h2>
					...
					<v-stepper-content step="6">
						<v-form class="form" ref="formStep6">
							...
							<template v-if="requestData.region_ab && requestData.education_form_id">
								...
								<v-btn color="primary" @click.stop="addQualificationBlock" v-if="requestData.payment_type_id == 2 && requestData.training_type_id == 1 && maxChoosedQualificationCoun > requestData.choosed_qualification.length">
			...
			async submitRequest() {
				...
				if(this.$refs.formStep6.validate()) {
					await this.$http.post('/request', this.requestData).then((response) => {
						...
						this.$router.push({ path: '/college/request/'+response.data.data.id });
					}).catch(() => {
						this.isLoading = false;
					})
				}
